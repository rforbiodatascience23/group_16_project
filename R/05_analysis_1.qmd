...
# All density plots
density_grid <- ggarrange(density_p1, density_p2, density_p3, density_p4, ncol=2, nrow=2, common.legend = TRUE, legend = "bottom")
density_grid
...
The data was elongated by inserting each numerical variable into one column, calles "numerical_variables".

```{r}
model_data <- data |>
  select(diagnose, age, trestbps, chol, thalach, oldpeak) |>
  mutate(diagnose = case_when(diagnose == "healthy" ~ 0,
                              diagnose == "sick" ~ 1)) |>
  pivot_longer(cols = c(age, trestbps, chol, thalach, oldpeak),
...
fitted_data <- model_data |>
  ...
  filter(term == "diagnose") |> ###intercept?
